...
static inline int pop(struct sensor_data_stack *stack)
{
	int ret;
	struct sensor_data *data =
		container_of(stack, struct sensor_data, stack);
...
static inline void push(struct sensor_data_stack *stack, int val)
{
	struct sensor_data *data =
		container_of(stack, struct sensor_data, stack);
...
/*
* all other operations except access operation
* the output is stored into stack
*/
static int data_op_others(struct sensor_data *data, struct data_action *action)
{
	int op1, op2;
	int result;
	int ret = 0;

	SENSOR_DBG(DBG_LEVEL4, data->dbg_on, "operands type: %x %x",
				action->operand1.type, action->operand2.type);

	/*operands*/
	op1 = data_operand(data, &action->operand1);
	op2 = data_operand(data, &action->operand2);
	result = data_op_array[action->op](op1, op2);
	push(&data->stack, result);
				...
				struct data_action *action)
{
	...
	if (action->op == OP_ACCESS)
		...
		return data_op_others(data, action);
	else {
		dev_err(&data->client->dev, "[%d]%s error\n",
					 __LINE__, __func__);
		return -EINVAL;
	}
}
...
static int register_sensor_driver(struct sensor_config *config)
{
	...
	strcpy(i2c_ids->name, config->name);
...
static void sensor_data_op_init(void)
{
	...
	data_op_array[OP_COMMA_EXPR] = data_op_comm;
}
