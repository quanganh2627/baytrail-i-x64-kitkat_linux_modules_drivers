					  ...
					  int value, const char *desc)
{
	...
	pr_err(DRVNAME "mdm_ctrl_configure_gpio GPIO %d (%s) to %d, result is %d", gpio, desc,value, ret);    
	ret = gpio_export(gpio, 1);
	if(ret)
        pr_info(DRVNAME": can't export gpio %d",gpio);
...
int cpu_init_gpio(void *data)
{
	...
	ret = mdm_ctrl_configure_gpio(100, 1, 0, "Powe Off");
			if (ret)
					goto free_ctx6;
