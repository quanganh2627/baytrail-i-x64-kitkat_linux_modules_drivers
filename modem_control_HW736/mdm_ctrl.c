...
static int mdm_ctrl_power_off(struct mdm_ctrl *drv)
{
	...
	gpio_direction_output(pwr_on, 0);
...
{
pr_info(DRVNAME ":fancy:mdm_ctrl_module_init");
	if(platform_driver_register(&mcd_driver))
        {
                pr_err("fancy: failed to register mcd_driver");
				 return -ENODEV;
        }
     return 0;
}
